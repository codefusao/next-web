...
const stockSchema = z.object({
	...
	highlight: z.boolean(),
	catalogItems: z.array(z.object({ quantity: z.number().int().positive() })),
...
function toInventoryItem(stock: z.infer<typeof stockSchema>): InventoryItem {
	return {
		...
		categoria: {
			...
			label: stock.product.category.name,
		},
		...
		availableQuantity: Math.max(
			stock.quantity -
				stock.catalogItems.reduce((total, item) => total + item.quantity, 0),
			0,
		),
...
export async function getInventory(
	...
	return {
		products: response.productStocks.map(toInventoryItem),
		meta: response.meta,
	};
}
...
export async function updateInventory(
	...
	const stock = await apiRequest(
		`/product-stock/${input.stockId}`,
		stockSchema,
		{
			method: "PUT",
			body: {
